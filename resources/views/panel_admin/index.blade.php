                    ...
                    <div class="row g-4 w-100">
                        @foreach($pedidos as $pedido)
                            <div class="col-12 col-md-6 col-lg-4">
                                <div class="card h-100 bg-dark text-white border-secondary shadow-sm p-3"
                                    style="min-width: 280px;">
                                    <div class="card-body d-flex flex-column text-center">

                                        <h4 class="card-title fw-bold mb-3">Pedido # {{ $pedidos->count() - $loop->index }}</h4>
                                        ...
                                        <p class="card-text fs-6 mb-4">
                                            ...
                                            @elseif($pedido->estado == 'carrito')
                                                <span class="badge bg-warning text-dark px-3 py-2 fs-6">En Carrito (Sin
                                                    pagar)</span>
                                            @else
                                                <span
                                                    class="badge bg-secondary px-3 py-2 fs-6">{{ ucfirst($pedido->estado) }}</span>
                                            @endif

                                            <span class="badge bg-light text-dark px-3 mt-2 py-2 fs-6">Entrega:
                                                {{ ucfirst($pedido->lugar_de_entrega) }}</span>
                                        </p>
                                ...
                                <div
                                    ...
                                    <ul class="list-group list-group-flush w-100">
                                        @foreach($usuarios as $users)
                                            @if($users->role == 'admin')
                                                <li
                                                    class="list-group-item bg-warning text-dark border-secondary border-radius-2 d-flex justify-content-between align-items-center">
                                                    <div>
                                                        <i class="bi bi-person-circle fs-4"></i> {{ $users->name }}
                                                        ({{ $users->email }})
                                                    </div>
                                                    @if(auth()->id() !== $users->id)
                                                        <form action="{{ route('usuarios.removeAdmin', $users->id) }}" method="POST"
                                                            class="m-0">
                                                            @csrf
                                                            @method('PATCH')
                                                            <button type="submit" class="btn btn-sm btn-danger fw-bold"
                                                                onclick="return confirm('¿Estás seguro de quitar el rol de administrador a este usuario?')">
                            ...
                            <div class="col-12 col-md-6 col-lg-4 tarjeta-producto">
                                <div class="card h-100 bg-dark text-white border-secondary shadow-sm">
                                    <div class="card-body d-flex flex-column text-center">
                                        ...
                                        <p class="card-text text-warning fs-5 mb-1">{{ $producto->stock }} unidades disponibles
                                        </p>
                                        <p class="card-text text-warning fs-5 fw-bold mb-4">${{ $producto->precio }}</p>

                                        <div class="mt-auto d-flex gap-2">
                                            <a href="{{ route('panel_admin.show', $producto->id) }}"
                                                class="btn btn-sm btn-info text-white w-100 fw-bold">Ver</a>
                                            <a href="{{ route('panel_admin.edit', $producto->id) }}"
                                                class="btn btn-sm btn-warning w-100 fw-bold">Editar</a>
                                            <form action="{{ route('panel_admin.destroy', $producto->id) }}" method="POST"
                                                class="w-100">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
                                                    class="btn btn-sm btn-danger w-100 fw-bold">Eliminar</button>
                                            </form>
                                        </div>
                                    </div>
                                </div>
...
@section('scripts')
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            ...
            // 1. Al cargar la página, buscamos si hay una pestaña guardada en memoria
            let pestañaGuardada = sessionStorage.getItem('pestañaActiva');
            if (pestañaGuardada) {
                let botonActivo = document.querySelector(`button[data-bs-target="${pestañaGuardada}"]`);
                if (botonActivo) {
                    // Usamos Bootstrap para simular el clic y mostrar la pestaña
                    let tab = new bootstrap.Tab(botonActivo);
                    tab.show();
                }
            }

            // 2. Guardamos la pestaña cada vez que el usuario cambia
